...
pprint (Or n m) = "(" ++ pprint n ++ " || " ++ pprint m ++ ")"
pprint (Not n) = "(" ++ " !" ++ pprint n ++ " )"
...
eval (Or t1 t2) = do v1 <- (eval t1)
                     v2 <- (eval t2)
                     return (liftBoolean (||) v1 v2)
eval (Not t) = do v <- (eval t)
                  return (Boolean (v == (Boolean False)))
